...
export default function Analytics() {
    ...
    (async () => {
      ...
      if (u) {
        try {
          ...
          setUserRoute((parsed?.routeName || parsed?.displayRouteName || '').toString().trim() || null);
        } catch {
          setUserRoute(null);
        }
      }
      ...
      try {
        setLoading(true);
        // pass routeName explicitly to server
        const data = await fetchAnalytics({ apiUrl, creds, query: { rangeDays, tz, routeName: userRoute || undefined } }); // pass tz
  ...
  // include userRoute in deps so charts update when user changes route
  }, [apiUrl, router, rangeDays, reloadKey, tz, userRoute]); // include tz
